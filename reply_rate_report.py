"""
Bison API — Domain & Mailbox level Reply Rate report.
Fetches Sent/Replied counts per mailbox, aggregates by domain, generates
...
MAILBOX_CSV = "reply_rate_by_mailbox.csv"
DOMAIN_CSV = "reply_rate_by_domain.csv"
FAILED_CSV = "reply_failed_mailboxes.csv"


def main():
    print(f"Reply rate report window: {START_DATE} to {END_DATE}")
    ...
    records, failed = fetch_all_mailbox_metrics(
        ...
        numerator_label="Replied", denominator_label="Sent",
        checkpoint_path="_checkpoint_reply.csv",
    )
    ...
    mailbox_report, domain_report, month_cols = build_mailbox_and_domain_reports(
        records, numerator_col_name="replied", rate_col_name="reply_rate_%"
    )
    ...
    if RESEND_API_KEY and RESEND_FROM_EMAIL and RESEND_TO_EMAIL:
        ...
        <div style="font-family:Arial,sans-serif;font-size:14px;color:#222;">
          <h2>Weekly Reply Rate Report</h2>
        ...
        send_resend_email(
            ...
            subject=f"Reply Rate Report ({START_DATE} to {END_DATE})",
            html_body=html,
